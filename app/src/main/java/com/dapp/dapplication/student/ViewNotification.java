...
public class ViewNotification extends BaseActivity {
    ...
    private void getDetails(StudentModel.Datum user) {
        ...
        RestBuilderPro.getService().viewnotification(hashMap).enqueue(new Callback<ResponseBody>() {
            @Override
                        ...
                        if (succ == 1) {
                            ...
                            if (jsonArray.length() != 0) {
                                for (int i = 0; i < jsonArray.length(); i++) {
                                    ...
                                    String name = object.getString("no_title");
                                    String content=object.getString("notification");
                                    //    String link = object.getString("as_date");
                                    String date = object.getString("no_date");


                                    viewlist.add(new ViewModel(name, content, "", TimeChange.parseDateToddMMyyyy(date)));

                                }
                                ViewAdapter adapter = new ViewAdapter(viewlist, ViewNotification.this, 3);
