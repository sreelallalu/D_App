...
public class ViewAssignmnet extends BaseActivity {
    ...
    private void getDetails(StudentModel.Datum user) {
        ...
        RestBuilderPro.getService().viewassignment(hashMap).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    dialog.dismiss();
               // as_branchid":1,"as_semid":1,"as_subjectid":1,"as_title":"test","as_content":"content","as_date":"2018-03-27 14:57:15","as_added":"admin"
                             ...
                             if (succ == 1) {
                                 ...
                                 if (jsonArray.length() != 0) {
                                     for (int i = 0; i < jsonArray.length(); i++) {
                                         ...
                                         String name = object.getString("as_title");
                                         String content=object.getString("as_content");
                                     //    String link = object.getString("as_date");
                                         String date = object.getString("as_date");


                                         viewlist.add(new ViewModel(name, content, "", TimeChange.parseDateToddMMyyyy(date)));

                                     }
                                     ViewAdapter adapter = new ViewAdapter(viewlist, ViewAssignmnet.this, 3);
