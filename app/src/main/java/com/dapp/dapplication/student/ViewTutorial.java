...
public class ViewTutorial extends BaseActivity {
    ...
    private void getDetails(StudentModel.Datum user) {
        ...
        RestBuilderPro.getService().viewtutorial(hashMap).enqueue(new Callback<ResponseBody>() {
            @Override
                        ...
                        if (succ == 1) {
                            ...
                            if (jsonArray.length() != 0) {
                                for (int i = 0; i < jsonArray.length(); i++) {
                                    ...
                                    String name = object.getString("tu_data");
                                    //  String content=object.getString("");
                                    String link = object.getString("tu_datalink");
                                    String date = object.getString("tu_date");


                                    viewlist.add(new ViewModel(name, "", link, TimeChange.parseDateToddMMyyyy(date)));

                                }
                                ViewAdapter adapter = new ViewAdapter(viewlist, ViewTutorial.this, 2);
