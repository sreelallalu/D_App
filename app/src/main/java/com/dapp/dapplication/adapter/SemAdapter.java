...
public class SemAdapter extends BaseAdapter {
    ...
    List<SemModel.Datum> arraylist;
    Context context;
    boolean state;
    private int langType;

    public SemAdapter(Context context,
                         List<SemModel.Datum> arrayofUsers) {
    ...
    public View getView(int positon, View view, ViewGroup parent) {
        ...
        SemModel.Datum users = arraylist.get(positon);

        TextView textView=view.findViewById(R.id.spinneride);
        textView.setText(users.getSeName());
